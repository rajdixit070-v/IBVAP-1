    ...
    if (!useFallbackMjpeg) {
      try {
        ws = new LiveFeedWebSocket(
          ...
          (err) => {
            console.warn(`WebSocket stream notice for ${camera.camera_id}, fallback to MJPEG:`, err);
            if (isSubscribed) {
      ...
      // Reconnect WebSocket stream after 10 seconds
          ...
          {camera.enabled !== false ? (
            <>
              {frameSrc ? (
                ...
                <img
                  ...
                  alt={camera.camera_name}
                  className="w-full h-full object-contain select-none"
                  onError={() => setUseFallbackMjpeg(true)}
                />
              )}
            ...
            <div className="flex flex-col items-center justify-center p-6 text-center text-slate-400 space-y-2">
              ...
              <p className="text-xs text-slate-500 max-w-xs">
                Camera stream is disabled by administrator policy.
              </p>
            </div>
          )}
          ...
          {showControls && (
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                className="p-1.5 bg-slate-900/80 text-slate-300 hover:text-sky-400 hover:bg-slate-800 rounded border border-slate-700/60 transition cursor-pointer"
              ...
              <button
                ...
                className="p-1.5 bg-slate-900/80 text-slate-300 hover:text-white hover:bg-slate-800 rounded border border-slate-700/60 transition cursor-pointer"
