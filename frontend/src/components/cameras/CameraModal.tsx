...
export const CameraModal: React.FC<CameraModalProps> = ({
  ...
  useEffect(() => {
    if (cameraToEdit) {
      ...
      setFormData({
        camera_id: `CAM-${postPrefix}-${randomSuffix}`,
  ...
  const handleTypeChange = (type: BorderEquipmentType) => {
    ...
    if (type === 'rtsp') {
      setFormData(prev => ({
        ...prev,
        stream_type: 'main',
        rtsp_url: prev.rtsp_url.startsWith('webcam://') || prev.rtsp_url.startsWith('udp://') || prev.rtsp_url.includes(':8080') ? '' : prev.rtsp_url
      }));
    } else if (type === 'nvr') {
      ...
      if (droneProtocol === 'qgc_udp') defaultDroneUrl = 'udp://0.0.0.0:5600';
  ...
  const handleDroneProtocolChange = (protocol: string) => {
    ...
    if (protocol === 'qgc_udp') url = 'udp://0.0.0.0:5600';
