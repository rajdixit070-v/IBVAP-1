...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, isOpen = true, onClose }) => {
  const { user } = useAuth();
        ...
        className={`
          ...
          ${
            isOpen
              ...
              : '-translate-x-full md:translate-x-0 md:w-0 opacity-0 pointer-events-none border-r-0'
          }
          h-full shrink-0 bg-[#090e17] border-r border-[#1e293b]/80 flex flex-col justify-between select-none overflow-hidden transition-all duration-300 shadow-2xl
        `}
      >
        {/* Mobile Header Bar with Close Button */}
        <div className="flex items-center justify-between px-3.5 py-3 border-b border-[#1e293b] md:hidden bg-[#070c16]">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center p-1">
              <img src="/logo.png" alt="Logo" className="w-full h-full object-contain" />
            </div>
            <span className="font-mono font-bold text-xs text-white tracking-wider">DEFENSE NAVIGATION</span>
          </div>
          {onClose && (
            <button
              onClick={onClose}
