...
const API_BASE = '/health';

export const healthService = {
  getServerHardwareTelemetry: async (): Promise<ServerHardwareTelemetry> => {
    const res = await api.get(`${API_BASE}/server`);
    return res.data;
  },

  getSystemHealth: async (): Promise<SystemHealthSummary> => {
    const res = await api.get(`${API_BASE}/system`);
    return res.data;
  },

  getCameraHealthList: async (): Promise<CameraHealthItem[]> => {
    const res = await api.get(`${API_BASE}/cameras`);
    return res.data;
  },

  getCameraHealthDetail: async (cameraId: string): Promise<CameraHealthItem> => {
    const res = await api.get(`${API_BASE}/cameras/${cameraId}`);
    return res.data;
  },

  setCameraPriority: async (cameraId: string, priority: string, reason?: string): Promise<any> => {
    const res = await api.post(`${API_BASE}/cameras/${cameraId}/priority`, {
      priority,
      reason,
  ...
  updateHealthConfig: async (payload: Partial<HealthConfig>): Promise<any> => {
    const res = await api.put(`${API_BASE}/config`, payload);
    return res.data;
  }
};
